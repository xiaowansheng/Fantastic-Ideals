...
<template>
  <nav role="navigation" class="nav" v-show="menuItems && menuItems.length">
    <ul class="nav-items">
          ...
          <nav class="submenu">
            <ul class="submenu-items">
              <li v-for="(subItem, subIndex) in item.submenu" :key="subIndex" class="submenu-item">
                ...
                <template v-else>
                  <a :href="subItem.url" target="_blank" class="submenu-link">{{
                    $t(subItem.label)
                  }}</a>
                </template>
              </li>
      ...
      <!-- 暂时不写语言切换，全按照系统语言设置 -->
      <!-- <li :class="'nav-item'" @click="onClickSubMenu($event)">
        <div class="nav-link">
          <svg-icon name="language" :iconStyle="'width:2.5rem;height:2.5rem'"></svg-icon>
        </div>
        <nav class="submenu">
          <ul class="submenu-items">
            <li class="submenu-item">
              <div class="submenu-link">简体中文</div>
              <div class="submenu-link">English</div>
            </li>
          </ul>
        </nav>
      </li> -->
...
.submenu-items {
  ...
  .submenu-link:hover {
    background-color: rgba($color: #3a4b62, $alpha: 0.2);
